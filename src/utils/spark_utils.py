...
def read_csv_to_dataframe(spark, file_path, schema=None, header=True):
    ...
    if schema:
        return spark.read.format("csv").option("header", header).schema(schema).load(file_path)
    else:
        return spark.read.format("csv").option("header", header).option("inferSchema", "true").load(file_path)
